...
- **Icons**: `@lucide/svelte` for **UI icons** — `import { Plus } from "@lucide/svelte"`, size with Tailwind
  (`class="size-4"`), tree-shaken (~400 B/icon after a one-time runtime). `@icons-pack/svelte-simple-icons`
  for **brand logos** — `import { SiBun } from "@icons-pack/svelte-simple-icons"`, size via the `size` prop;
  `color` defaults to `currentColor` so Tailwind text utilities tint them. Avoid Vite-only icon tooling like
  `unplugin-icons` — Bun's bundler isn't a supported target.
